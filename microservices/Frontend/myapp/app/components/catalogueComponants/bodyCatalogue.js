...
const bodyCatalogue = () => {
    ...
    //             productId: product.id,  // Assure-toi que c'est bien "id" et non "_id"
    //             quantity: 1, 
    //         });
    ...
    //         alert(`Erreur : ${error.response?.data?.error || "Impossible d'ajouter le produit."}`);
    //     }
    // };
    ...
    const addToCart = async (productId) => {
        try {
            console.log("Tentative d'ajout au panier...", productId);
    
            const response = await axios.post("http://localhost:5001/api/cart", {
                productId: productId,  // Assure-toi que c'est bien "id" et non "_id"
                quantity: 1, 
            });
